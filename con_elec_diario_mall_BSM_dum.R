...
mod2<-SSModel(elma_e~SSMtrend(degree=2,Q=list(matrix(NA),matrix(NA)))+SSMseasonal(period=7,sea.type = "dummy",Q=NA),H=NA)
...
QQ=matrix(fit_mod2$model$Q,3,3)

ts.plot(elma_e,out_mod2$a[,1]+out_mod2$a[,3],col=1:2) # prediccion con dummy
...
mod_pred<-SSModel(elma_e~SSMtrend(degree=2,Q=list(matrix(QQ[1,1]),matrix(QQ[2,2])))+SSMseasonal(period=7,sea.type = "dummy",Q=QQ[3,3]),H=h11)
